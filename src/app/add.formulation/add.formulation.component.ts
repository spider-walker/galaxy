...
@Component({
    selector: 'app-add.formulation',
    templateUrl: './add.formulation.component.html',
    styleUrls: ['./add.formulation.component.css']
})
export class AddFormulationComponent implements OnInit {
    createItem: FormGroup;
    loading = false;
    submitted = false;
    finishedGoodsItemsColors: Array<String> = ["Red", "Green ", "Beige", "Blue", "Yellow"];
    finishedGoodsItems: Item[];
    rawMaterialstems:Item[];
    constructor(
    ...
    ngOnInit() {
        this.createItem = this.formBuilder.group({
            ...
            theoritical_sg: ['0', Validators.required],
            measured_sg: ['0', Validators.required],
            production_loss: ['0', Validators.required],
